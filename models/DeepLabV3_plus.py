...
def Upsampling(inputs,feature_map_shape):
    return tf.compat.v1.image.resize_bilinear(inputs, size=tf.cast(feature_map_shape, tf.int32))
...
def AtrousSpatialPyramidPoolingModule(inputs, depth=256):
    ...
    image_features = tf.compat.v1.reduce_mean(inputs, [1, 2], keep_dims=True)
