...
parser.add_argument('--dataset', type=str, default="TomatoDataset", help='Dataset you are using.')
...
loss = tf.compat.v1.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=network, labels=net_output))
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    for i in range(num_iters):
        ...
        for j in range(args.batch_size):
            ...
            id = id_list[index]
            #print("Name = %s" % (train_input_names[id]))
            #print("Name = %s" % (train_output_names[id]))
